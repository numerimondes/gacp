...
show_help() {
    echo -e "${CYAN}gacp - Git Add Commit Push automation${NC}"
    echo ""
    echo -e "${WHITE}Usage:${NC}"
    echo "  gacp          Individual commits (default)"
    echo "  gacp -g       Grouped commit (all files in one commit)"
    echo "  gacp -h       Show this help"
    echo ""
    echo -e "${WHITE}Description:${NC}"
    echo "  Intelligent Git workflow automation with conventional commits"
    echo "  Automatically stages, commits with smart messages, and pushes"
    echo "  Optimized for Laravel applications with context-aware commit messages"
}
